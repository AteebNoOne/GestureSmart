// permissions.ts
import { PermissionsAndroid, Platform, Rationale, Alert, Linking, Permission } from 'react-native';

// Type for permission request result
type PermissionStatus = 'granted' | 'denied' | 'never_ask_again';

// Interface for tracking permission results
interface TrackingPermissionResult {
  backgroundPermissionGranted: boolean;
  cameraPermissionDenied: boolean;
}

// Interface for tracking permission handlers
interface TrackingPermissionHandlers {
  setBackgroundPermissionGranted: (value: boolean) => void;
...
// Tracking permissions (for eye/gesture tracking)
export const requestTrackingPermissions = async (
  handlers: TrackingPermissionHandlers,
  featureType: 'eye tracking' | 'gesture tracking'
  ...
  try {
    if (Platform.OS === 'android') {
      ...
      if (Platform.Version && Number(Platform.Version) >= 34) { // Android 14
        permissions.push(
      ...
      if (allGranted) {
        handlers.setBackgroundPermissionGranted(true);
        handlers.setCameraPermissionDenied(false);
        ...
        if (cameraPermission === PermissionsAndroid.RESULTS.DENIED ||
          ...
          handlers.setCameraPermissionDenied(true);
        }

        Alert.alert(
          'Permissions Required',
          `Android 14 requires all permissions for background ${featureType}. Please enable them in Settings.`,
          [
      ...
      return {
        backgroundPermissionGranted: allGranted,
        cameraPermissionDenied: !allGranted && results['android.permission.CAMERA'] !== PermissionsAndroid.RESULTS.GRANTED
      };
    } else {
      handlers.setBackgroundPermissionGranted(true);
      return {
...
export const showTrackingPermissionAlert = (
  ...
  Alert.alert(
    ...
    Platform.Version && Number(Platform.Version) >= 34
      ? `Android 14 requires camera and foreground service permissions for background ${featureType}.`
      : `Please grant all required permissions for background ${featureType}.`,
    [
