...
public class GestureService extends Service {
    ...
    private final BroadcastReceiver cursorModeReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if ("com.ateebnoone.gesturesmartv2.CURSOR_MODE".equals(intent.getAction())) {
    ...
    public void onCreate() {
        ...
        try {
            ...
            // FIXED: Register receiver with proper flags for Android 14+
            IntentFilter filter = new IntentFilter("com.ateebnoone.gesturesmartv2.CURSOR_MODE");
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                // For Android 13+ (API 33+), use RECEIVER_NOT_EXPORTED for security
                registerReceiver(cursorModeReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
            } else {
                registerReceiver(cursorModeReceiver, filter);
            }
            Log.i(TAG, "Cursor mode broadcast receiver registered");

            // Initialize React Native modules with better error handling
            try {
                ...
                Log.e(TAG, "Error initializing React Native modules: " + e.getMessage());
                // Continue without React modules for now
            }
    ...
    private void initializeMediaPipe() {
        try {
            HandsOptions options = HandsOptions.builder()
                    ...
                    .setMinDetectionConfidence(0.6f) // Lowered for better detection
                    .setMinTrackingConfidence(0.6f) // Lowered for better tracking
                    .build();
            ...
            hands.setResultListener(result -> {
                try {
                    if (result != null && !result.multiHandLandmarks().isEmpty()) {
                        ...
                        resetGestureTracking();
                        // Send no hands event more frequently for better feedback
                        if (gestureModule != null && System.currentTimeMillis() - lastHandDetectionTime > 1000) {
                            processHandler.post(() -> gestureModule.sendHandDetectionEvent("no_hands", 0, 0.0f));
                            lastHandDetectionTime = System.currentTimeMillis();
                        }
                    }
                } catch (Exception e) {
                    Log.e(TAG, "Error in MediaPipe result listener: " + e.getMessage());
                }
            });
        } catch (Exception e) {
    ...
    private void detectAndEmitGesture(HandsResult result) {
        ...
        try {
            ...
            // Send hand detection event with current timestamp
            if (gestureModule != null) {
                processHandler.post(
    ...
    private void startBackgroundThread() {
        ...
        Log.i(TAG, "Background thread started with max priority");
    }
    ...
    private Notification createNotification(String text) {
        ...
        // Android 14+ specific optimizations
        if (Build.VERSION.SDK_INT >= 34) {
    ...
    private void handleCameraLoss() {
        if (isCameraLost || restartCount >= MAX_RESTART_ATTEMPTS) {
            ...
            updateNotification("Camera recovery failed - check device");
            return;
        }
    ...
    private void createCameraPreviewSession() {
        try {
            // Enhanced ImageReader settings for better performance
            imageReader = ImageReader.newInstance(640, 480, ImageFormat.YUV_420_888, 3); // Increased buffer count
            ...
            // Enhanced camera settings for better performance
                    ...
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession session) {
                            ...
                            try {
                                ...
                                updateNotification("Camera active - detecting gestures");
    ...
    private void startCamera() {
        ...
        try {
            // Enhanced permission checking for Android 14+
            ...
            // Additional permission checks for Android 14+
            if (Build.VERSION.SDK_INT >= 34) {
                ...
                for (String permission : requiredPermissions) {
                    if (ContextCompat.checkSelfPermission(this, permission) != PackageManager.PERMISSION_GRANTED) {
                        Log.e(TAG, "Required permission not granted: " + permission);
                        updateNotification("Permissions required for Android 14+");
                        return;
                    }
                }
            }
    ...
    public void onDestroy() {
        ...
        if (hands != null) {
            try {
                ...
                Log.e(TAG, "Error closing MediaPipe hands: " + e.getMessage());
            }
        }
    ...
    public void onLowMemory() {
        ...
        // Reduce MediaPipe confidence thresholds temporarily
        if (hands != null) {
            try {
                // Could reduce processing frequency here
                Log.i(TAG, "Reducing processing load due to low memory");
            } catch (Exception e) {
                Log.e(TAG, "Error handling low memory: " + e.getMessage());
            }
        }
        super.onLowMemory();
    }
